...
void initialize_parameters(vector<double> &abc_args, SimParameters &simpar) {
    ...
    bool use_vac = (bool) abc_args[3];
    double vac_eff1 = abc_args[4];
    double vac_cov1 = abc_args[5];
...
vector<double> simulator(vector<double> args, const unsigned long int rng_seed, const unsigned long int serial, const ABC::MPI_par* mp) {
    ...
    initialize_parameters(args, simpar);

    const int net_replicate = (int) args[0];
    const int epi_replicate = (int) args[1]; // intervention replicate
    const bool use_clustering = (bool) args[2];

    simpar.seed = epi_replicate;

    //map<Node*, int> level_of;
    //Network* net = generate_ebola_network(netpar, level_of);
    Network* net = new Network(Network::Undirected);
...
//    cerr << "Line " << net_replicate << " was " << network_filename << endl;
    net->read_edgelist(network_dir + network_filename);
    assert(net->size() > 0);
    //Node* p_zero = net->get_nodes()[0];          // not elegant, but works for now
    Node* p_zero = net->get_node_by_name("0");
    map<const Node*, double> level_of = p_zero->min_path_map();
