...
.args <- c(list.files(pattern="\\d+\\.rds", full.names = T), "thing.png")
...
asymp <- bnd[clusters > 90, .(RR=mean(md.RR)), by=c(setdiff(key(samples),"clusters")) ]

effplot2 <- ggplot(bnd[bveff %in% c(0.2,0.4,0.6,0.8)]) +
