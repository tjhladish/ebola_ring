...
class Particle {
  private:
    ...
    double _y;
    int _id;
    std::vector<Link*> _edgesIn;
    std::vector<Link*> _edgesOut;
  ...
  public:
    ...
    std::vector<Link*> edgesIn() { return _edgesIn; }
    std::vector<Link*> edgesOut() { return _edgesOut; }
    void addLinkTo(Particle* o) { Link* link = new Link(this, o); _edgesOut.push_back(link); o->addLink(link); }
    void addLink(Link* link) { _edgesIn.push_back(link); }

    Particle (int id, double xi, double yi ) : _id(id), _x(xi), _y(yi), vx(0), vy(0), fx(0), fy(0), px(xi), py(yi) {}
